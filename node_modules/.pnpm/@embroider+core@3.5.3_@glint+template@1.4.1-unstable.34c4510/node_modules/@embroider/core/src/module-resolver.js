...
class Resolver {
    ...
    *internalResolve(request, defaultResolve) {
    ...
    nodeResolve(specifier, fromFile) {
        let resolution = this.resolveSync(new NodeModuleRequest(specifier, fromFile, false, undefined), request => {
            ...
            return (0, node_resolve_1.resolve)(request.specifier, request.fromFile, this.options.resolvableExtensions);
        });
        switch (resolution.type) {
            case 'not_found':
                return resolution;
    ...
    generateFastbootSwitch(request) {
        ...
        if ((0, shared_internals_1.packageName)(request.specifier)) {
        ...
        let appRelativePath = (0, shared_internals_2.explicitRelative)(pkg.root, (0, path_1.resolve)((0, path_1.dirname)(request.fromFile), request.specifier));
        if (engineConfig) {
            for (let candidate of this.withResolvableExtensions(appRelativePath)) {
                ...
                if (fastbootFile) {
                    if (fastbootFile.shadowedFilename) {
                        ...
                        return logTransition('unshadowed app fastboot', request, request.alias(fastbootFile.localFilename).rehome((0, path_1.resolve)(pkg.root, 'package.json')));
                    }
                }
            }
        }
        ...
        if (pkg) {
            ...
            if (engineConfig) {
                ...
                if (fastbootFile && fastbootFile.shadowedFilename) {
                    ...
                    return logTransition('matched app entry', request, 
    ...
    *componentJSCandidates(inPackageName) {
    ...
    owningEngine(pkg) {
        let owningEngine = this.options.engines.find(e => pkg.isEngine() ? e.root === pkg.root : e.activeAddons.find(a => a.root === pkg.root));
        if (!owningEngine) {
...
`);
    ...
    *withResolvableExtensions(filename) {
