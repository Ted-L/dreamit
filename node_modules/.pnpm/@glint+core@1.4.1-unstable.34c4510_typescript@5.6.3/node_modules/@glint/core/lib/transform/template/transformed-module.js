...
export default class TransformedModule {
    ...
    toDebugString() {
        let mappingStrings = this.correlatedSpans.map((span) => span.glimmerAstMapping?.toDebugString({
    ...
    findTemplateAtOriginalOffset(originalFileName, originalOffset) {
        ...
        assert(correlatedSpan.glimmerAstMapping?.sourceNode.type === 'TemplateEmbedding' &&
     ...
     */
    toVolarMappings(filenameFilter) {
        const sourceOffsets = [];
        const generatedOffsets = [];
        const lengths = [];
        const push = (sourceOffset, generatedOffset, length) => {
