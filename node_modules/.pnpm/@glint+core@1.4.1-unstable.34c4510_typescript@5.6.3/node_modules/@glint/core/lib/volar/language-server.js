...
connection.onInitialize((parameters) => {
    ...
    return server.initialize(parameters, project, 
...
function filterAndAugmentDiagnostics(context, document, diagnostics) {
    if (!diagnostics) {
    ...
    let cachedVirtualCode = undefined;
    const fetchVirtualCode = () => {
    ...
    const augmentedDiagnostics = diagnostics.map((diagnostic) => {
        ...
        return augmentDiagnostic(diagnostic, mappingForDiagnostic);
    });
    ...
    augmentedDiagnostics.forEach((diagnostic) => {
        ...
        let appliedDirective = undefined;
        if (transformedModule) {
    ...
    for (let directive of unusedExpectErrors) {
        ...
        allDiagnostics.push({
            ...
            range: vscode.Range.create(offsetToPosition(document.getText(), directive.areaOfEffect.start), offsetToPosition(document.getText(), directive.areaOfEffect.end)),
