...
import { curry, createCapturedArgs, EMPTY_POSITIONAL } from '@glimmer/runtime';
 ...
 */
function curryComponent(componentKlass, namedArgs, owner) {
  if (!namedArgs || !componentKlass || !owner) {
    throw new Error('curryComponent requires a component class, named arguments, and an owner');
  }
  let namedDict = dict();
  for (const key of Object.keys(namedArgs)) {
    namedDict[key] = createComputeRef(() => namedArgs[key]);
  }
  return curry(ComponentCurriedType, componentKlass, owner, createCapturedArgs(namedDict, EMPTY_POSITIONAL), false);
}
...
setHelperManager(owner => new CurryComponentHelperManager(owner), curryComponent);

export { curryComponent as default };
